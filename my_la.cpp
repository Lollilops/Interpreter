...
enum type_of_lex {
    LEX_NULL,                                                                                               /*0*/
    LEX_AND, LEX_REAL, LEX_DO, LEX_ELSE, LEX_IF, LEX_FALSE, LEX_INT,LEX_NOT, LEX_OR, LEX_PROGRAM,           /*10*/
...
Lex Scanner::get_lex() {
    enum    state { H, IDENT, NUMB, SLASH, COM, COM_TIMES, ALE, NEQ, STR};
    int     d, j;
    int flag_real = 0;
    string  buf;
    state   CS = H;
            ...
            case H:
                if( c == ' ' || c == '\n' || c == '\r' || c == '\t' ){
                    ...
                    if ( (j = search(buf, TD)) ){
                        ...
                        char s[BUFSIZE];
                        sprintf(s, "%s (%d): Непонятный тип. Строка %d, cтолбец %d\n", __FILE__, __LINE__, row, column);
                        string str = string(s);
                        throw str;
                    }
                }
